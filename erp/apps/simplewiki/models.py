...
class RootArticleNotFound(Exception):
    pass

class Article(models.Model):
    """Wiki article referring to Revision model for actual content.
       'slug' field should be maintained centrally, since users
       aren't allowed to change it, anyways.
    """
    ...
    def get_root(cls):
        ...
        try:
            return Article.objects.get(slug__iexact=WIKI_ROOT_SLUG)
        except:
            raise RootArticleNotFound()
...
class Revision(models.Model):
    ...
    def save(self, **kwargs):
        ...
        def url_builder(label, base, end):
            if Article.objects.filter(slug__iexact=label).exists():
                wikilink_url = reverse('wiki_view', args=(label,))
            else:
                wikilink_url = reverse('wiki_create', args=(label,))
            return wikilink_url
