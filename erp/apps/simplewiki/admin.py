...
from erp.apps.simplewiki.models import Article, Revision, Permission, ArticleAttachment
...
class ArticleAdminForm(forms.ModelForm):
    def clean(self):
        ...
        if not self.instance.pk:
            ...
            if Article.objects.filter(slug__iexact=slug):
                raise forms.ValidationError(_('Article slug must be '
                                              'unique.'))
        return cleaned_data
    class Meta:
        model = Article
...
class ArticleAdmin(admin.ModelAdmin):
    list_display = ('title', 'created_by', 'slug', 'modified_on',)
    search_fields = ('slug',)
